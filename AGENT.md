...
1. **Solo-author git as WYR186.** Never add `Co-Authored-By:`. Never mention Claude / Anthropic / "generated by AI" anywhere. This repo has a **local** `user.name = WYR186` / `user.email = 98409180+WYR186@users.noreply.github.com` set via `git config user.name/email` (matches the GitHub account that owns the repo). Don't fall back to the global identity (`AaronWonMac`) — every commit must say `WYR186`. Verify with `git log -1 --format='%an <%ae>'` after committing.
